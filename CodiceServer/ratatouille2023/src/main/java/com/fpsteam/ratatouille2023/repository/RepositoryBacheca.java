...
public interface RepositoryBacheca extends JpaRepository<Bacheca,BachecaId> {
	@Query(
			value="SELECT a.id_avviso, a.corpo, a.oggetto, a.data, a.autore FROM bacheca as b, avviso as a WHERE b.id_utente = ?1 AND b.id_avviso = a.id_avviso AND b.visualizzato = ?2",
			nativeQuery = true
			)
	List<Object> findByVisualizzato(int uid, boolean visualizzato);
	
	@Query(
			value="SELECT a.id_avviso, a.corpo, a.oggetto, a.data, a.autore FROM bacheca as b, avviso as a WHERE b.id_utente = ?1 AND b.id_avviso = a.id_avviso AND b.visibile = 1 AND b.visualizzato = 1",
			nativeQuery = true
			)
	List<Object> findVisibili(int uid);
	
	@Query(
			value = "SELECT a.id_avviso, a.corpo, a.oggetto, a.data, a.autore FROM bacheca as b, avviso as a WHERE b.id_utente = ?1 AND b.id_avviso = a.id_avviso AND b.visibile = 0",
			nativeQuery = true
			)
